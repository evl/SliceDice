if select(2, UnitClass("player")) == "DEATHKNIGHT" then
	...
	local bloodPlagueBar = evl_SliceDice:CreateBar("target", "Blood Plague", bloodPlagueDuration + diseaseDurationBonus, 6)
	bloodPlagueBar.colors = {{200/255, 0/255, 0/255}}
	bloodPlagueBar.label:Hide()

	-- Frost Fever
	local frostFeverDuration = 15 + diseaseDurationBonus
	local frostFeverBar = evl_SliceDice:CreateBar("target", "Frost Fever", frostFeverDuration + diseaseDurationBonus, 6)
